...
import { Otp, OtpType } from './schema/otp.schema';
import { OtpNotificationService } from './otp-notification.service';
import { User } from 'src/users/schema/user.schema'; // تأكد من المسار الصحيح للـ User class
...
export class OtpService {
  ...
  constructor(
    @InjectModel(Otp.name) private readonly otpModel: Model<Otp>,
    // 1. حقن موديل المستخدم هنا لعمل التحديث
  ...
  async createOtp(
    ...
    const identifier = options.email || options.userId?.toString();
    try {
      await this.markExistingOtpsUsed(type, options);
      ...
      this.logger.log(`OTP sent successfully to ${identifier}`);
      return plainCode;
    } catch (error) {
      this.logger.error(`Failed to create OTP: ${error.message}`);
      throw new InternalServerErrorException('Failed to send OTP.');
    }
  }

  /**
   * التحقق وتحديث حالة المستخدم فوراً
   */
  async validateOtp(
    inputCode: string,
    type: OtpType,
    options: { userId?: string | Types.ObjectId; email?: string },
  ): Promise<boolean> {
    const filter = this.buildOtpFilter(type, options);
    ...
    try {
      ...
      // 2. تحديث حالة المستخدم في الداتابيز (هذا ما سيحل مشكلة الـ 401)
      if (options.userId) {
        await this.userModel.findByIdAndUpdate(options.userId, {
          $set: { isVerified: true },
        });
      ...
      // 3. تعليم الـ OTP كـ مستخدم
      otp.isUsed = true;
      await otp.save();

      this.logger.log(
        `OTP validated and user verified for ${options.email || options.userId}`,
      );
      return true;
    } catch (error) {
      if (error instanceof HttpException) throw error;
